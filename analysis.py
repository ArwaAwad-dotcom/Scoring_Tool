...
df=get_relevant_data(file_name)
dg=get_relevant_data(file_name_two)
all_data=pd.concat([df,dg])
set_data=set(list(all_data['Level 2']))





#Proccess the baseline data
bas_data=pd.read_excel(baseline_data,sheet_name="CF-Baselining")
...
set_data_bas=set(list(bas_data_data_frame['Level 2']))
lo=set_data-set_data_bas
...
matrix_level_pos=np.zeros((len(all_data_levels_two),len(list_dates_all_position)))
print("Number of values",len(all_data_levels_two))
for dd in all_data_levels_two:
    #print(dd)
...
for i, row in enumerate(matrix_level_pos):
